"""
SpaGCN on the BC (human breast cancer, Visium) dataset.
...
Expected layout:
  ...
  BC/section1/section1_full_image.tif          (optional, see below)

Outputs written to dir_output:
  spagcn_bc.csv               refined cluster labels (barcode, refined_pred)
  spagcn_embeddings_bc.csv    SpaGCN latent embedding, spots x 50
  spagcn_aris_bc.txt          one "HBRC1 <ARI>" line appended per run
"""
...
adata = load_BC(root_dir=dir_input, section_id=section_id)
...
adata.obs["refined_pred"].to_csv(os.path.join(dir_output, "spagcn_bc.csv"))
np.savetxt(os.path.join(dir_output, "spagcn_embeddings_bc.csv"),
           np.asarray(adata.obsm["X_spagcn"]), delimiter=",")
with open(os.path.join(dir_output, "spagcn_aris_bc.txt"), "a+") as fp:
