...
class SpaGCN(object):
    ...
    def predict(self, adata=None):
        ...
        prob=q.detach().numpy()
        # Upstream discards the latent matrix z. Passing an AnnData in stores it as
        # adata.obsm['X_spagcn'] and returns the object, so the embedding can be saved
        # alongside the labels. Called with no argument the behaviour is unchanged,
        # which is what util.search_res relies on.
        if adata is None:
            return y_pred, prob
        adata.obsm['X_spagcn']=z.detach().cpu().numpy()
        return y_pred, prob, adata
