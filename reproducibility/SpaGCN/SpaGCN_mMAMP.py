"""
SpaGCN on the mMAMP (mouse brain anterior, "MA") dataset.
...
Expected layout:
  ...
  mMAMP/MA/MA_full_image.tif                  (optional, see below)

Outputs written to dir_output:
  spagcn_mMAMP.csv              refined cluster labels (barcode, refined_pred)
  spagcn_embeddings_mMAMP.csv   SpaGCN latent embedding, spots x 50
  spagcn_aris_mMAMP.txt         one "mABC <ARI>" line appended per run
"""
...
adata = load_mMAMP(root_dir=dir_input, section_id=section_id)
...
adata.obs["refined_pred"].to_csv(os.path.join(dir_output, "spagcn_mMAMP.csv"))
np.savetxt(os.path.join(dir_output, "spagcn_embeddings_mMAMP.csv"),
           np.asarray(adata.obsm["X_spagcn"]), delimiter=",")
with open(os.path.join(dir_output, "spagcn_aris_mMAMP.txt"), "a+") as fp:
    fp.write("mABC " + str(ARI) + "\n")
