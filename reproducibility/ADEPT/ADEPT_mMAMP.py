"""
ADEPT on the mMAMP (mouse brain anterior, "MA") dataset.
...
Outputs written to dir.output:
  mMAMP_adept.txt              final cluster labels (one per spot)
  adept_embeddings_mMAMP.csv   final latent embedding
  adept_aris_mMAMP.txt         one "mMAMP_MA <ARI>" line appended per run
"""
...
cache_file = os.path.join(cache_dir, "MA" + section_id + ".txt")
...
np.savetxt(os.path.join(dir_output, "mMAMP_adept.txt"),
           adata_out.obs[label_key], fmt="%s")
np.savetxt(os.path.join(dir_output, "adept_embeddings_mMAMP.csv"),
           np.asarray(adata_out.obsm[embed_key]), delimiter=",")
with open(os.path.join(dir_output, "adept_aris_mMAMP.txt"), "a+") as fp:
    fp.write("mMAMP_" + section_id + " " + str(ARI) + "\n")
