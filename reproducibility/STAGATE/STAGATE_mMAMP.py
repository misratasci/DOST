"""
STAGATE on the mMAMP (mouse brain anterior, "MA") dataset.
...
Outputs written to dir_output:
  stagate_mMAMP.csv              mclust labels (barcode, mclust)
  stagate_embeddings_mMAMP.csv   STAGATE embedding, spots x 30
  stagate_aris_mMAMP.txt         one "mABC <ARI>" line appended per run
"""
...
ad.obs["mclust"].to_csv(os.path.join(dir_output, "stagate_mMAMP.csv"))
np.savetxt(os.path.join(dir_output, "stagate_embeddings_mMAMP.csv"),
           np.asarray(ad.obsm["STAGATE"]), delimiter=",")
with open(os.path.join(dir_output, "stagate_aris_mMAMP.txt"), "a+") as fp:
