...
def mclust_R(adata, num_cluster, modelNames='EEE', used_obsm='STAGATE', random_seed=2020):
    ...
    # Ensure conversion is active
    numpy2ri.activate()
    
    # Load library
    robjects.r.library("mclust")
    
    # 1. Force the data into a pure, contiguous numpy array (No Pandas leftover)
    # We use 'embedding' because your print showed that's where the data is.
    data = np.ascontiguousarray(adata.obsm[used_obsm], dtype='float64')
    
    # 2. Assign to R and use R's native 'unname' to strip metadata
    robjects.r.assign("raw_data", data)
    robjects.r("clean_data <- unname(as.matrix(raw_data))")
    
    # 3. Explicitly convert num_cluster to an R integer
    robjects.r.assign("k", int(num_cluster))
    robjects.r.assign("model_name", modelNames)
    
    # 4. Execute Mclust entirely within the R string to avoid rpy2 argument wrapping errors
    print(f"Running Mclust for {num_cluster} clusters...")
    res = robjects.r("Mclust(clean_data, G=k, modelNames=model_name)")
    
    # 5. Extract the classification (the second to last element in the Mclust list)
    # In R, it's res$classification. In rpy2, we can grab it by name:
