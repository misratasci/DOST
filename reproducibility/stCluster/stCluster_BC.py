"""
stCluster on the BC (human breast cancer, Visium) dataset.
...
Outputs written to dir_output:
  stcluster_bc_section1.csv             mclust labels (barcode, mclust)
  stcluster_embeddings_bc_section1.csv  stCluster embedding, spots x 30
  stcluster_aris_bc.txt                 one "section1 <ARI>" line appended per run
"""
...
ad.obs["mclust"].to_csv(
    os.path.join(dir_output, "stcluster_bc_{}.csv".format(section_id)))
np.savetxt(os.path.join(dir_output,
                        "stcluster_embeddings_bc_{}.csv".format(section_id)),
           np.asarray(ad.obsm["embedding"]), delimiter=",")
with open(os.path.join(dir_output, "stcluster_aris_bc.txt"), "a+") as fp:
